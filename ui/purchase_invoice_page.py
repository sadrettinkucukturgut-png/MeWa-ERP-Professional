...
class PurchaseInvoicePage(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Alış Faturaları")
    ...
    def _setup_ui(self):
        ...
        self.page_title = QLabel("🧾 Alış Faturaları")
        ...
        self.search_input.setPlaceholderText("Fatura No, Tedarikçi, Sipariş, Mal Kabul veya Durum")
        ...
        self.action_new = QAction("➕ Yeni Fatura", self)
        ...
        self.table.setObjectName("purchaseInvoiceTable")
        self.column_labels = [
            "Fatura No",
            "Fatura Tarihi",
            "Tedarikçi",
    ...
    def load_invoices(self, keyword: str = ""):
        rows = PurchaseInvoiceModel.list_invoices(keyword)
        self._rows = rows
    ...
    def _cancel_selected(self):
        ...
        try:
            PurchaseInvoiceModel.cancel_invoice(invoice_no)
            self.load_invoices(self.search_input.text())
        except Exception as exc:
    ...
    def _restore_column_visibility(self):
        for index, _ in enumerate(self.column_labels):
            visible = self.settings.value(f"purchase_invoice_columns/{index}", True)
            self.table.setColumnHidden(index, not visible)
    ...
    def _export_to_excel(self):
        ...
        ExcelService.export_excel(
            ...
            headers,
            rows,
            "Alis_Fatura_Listesi.xlsx",
            sheet_title="Alış Faturaları",
