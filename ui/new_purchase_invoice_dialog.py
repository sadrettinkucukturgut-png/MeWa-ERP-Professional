...
class NewPurchaseInvoiceDialog(BaseDocumentDialog):
    ...
    def _setup_ui(self):
        ...
        self.setTabOrder(self.product_table, self.preview_btn)
        self.setTabOrder(self.preview_btn, self.save_btn)
        self.setTabOrder(self.save_btn, self.save_close_btn)
        self.setTabOrder(self.save_close_btn, self.cancel_btn)
    ...
    def _on_save(self):
        self._save_document(close_on_success=False)
    ...
    def _initialize_preview_state(self):
        self.preview_btn.setEnabled(self._has_persisted_document)
    ...
    def _get_preview_controller(self) -> DocumentPreviewController:
        if self._preview_controller is None:
            self._preview_controller = DocumentPreviewController(
                ...
                template_provider=self._build_saved_preview_template,
            )
    ...
    def _build_saved_preview_template(self) -> DocumentTemplate:
        ...
        for item in invoice.get("items", []):
            ...
            items.append(
                DocumentLineItem(
                    ...
                    description=str(item.get("product_name") or ""),
                    quantity=f"{qty:.3f}".rstrip("0").rstrip("."),
                    unit=str(item.get("unit") or ""),
                    unit_price=f"{unit_price:.2f}",
                    discount=f"{discount_percent:.2f}%",
        ...
        return DocumentTemplate(
            ...
            vat_total=f"{float(invoice.get('vat_total') or 0):.2f}",
            grand_total=f"{float(invoice.get('grand_total') or 0):.2f}",
